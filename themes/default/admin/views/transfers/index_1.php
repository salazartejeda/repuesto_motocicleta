...
<script>
    $(document).ready(function () {
        oTable = $('#TOData').dataTable({
            ...
            'sAjaxSource': '<?= admin_url('transfers/getTransfers_1') ?>',
            ...
            "aoColumns": [{"bSortable": false,"mRender": checkbox}, {"mRender": fld}, null, null, null, null, null, null, null, {"mRender": currencyFormat}, {"mRender": currencyFormat}, {"mRender": currencyFormat}, {"mRender": row_status}, {"bSortable": false,"mRender": attachment}, {"bSortable": false}],
            ...
            "fnFooterCallback": function (nRow, aaData, iStart, iEnd, aiDisplay) {
                ...
                for (var i = 0; i < aaData.length; i++) {
                    row_total += parseFloat(aaData[aiDisplay[i]][9]);
                    tax += parseFloat(aaData[aiDisplay[i]][10]);
                    gtotal += parseFloat(aaData[aiDisplay[i]][11]);
                }
                var nCells = nRow.getElementsByTagName('th');
                nCells[9].innerHTML = currencyFormat(formatMoney(row_total));
                nCells[10].innerHTML = currencyFormat(formatMoney(tax));
                nCells[11].innerHTML = currencyFormat(formatMoney(gtotal));
            }
        }).fnSetFilteringDelay().dtFilter([
            {column_number: 1, filter_default_label: "[<?=lang('date');?> (yyyy-mm-dd)]", filter_type: "text", data: []},
            {column_number: 2, filter_default_label: "[<?=lang('Traslado_No');?>]", filter_type: "text", data: []},
            {
                column_number: 3,
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-star-o"></i><?= lang('Porductos Transferidos'); ?></h2>
