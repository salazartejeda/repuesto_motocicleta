...
<div class="modal-dialog no-modal-header" role="document"><div class="modal-content"><div class="modal-body">
    ...
    <div id="wrapper">
        <div id="receiptData">
                ...
                <table class="table table-condensed">
                    <tbody>
                        ...
                        foreach ($rows as $row) {
                            ...
                            echo '<tr><td class="no-border border-bottom">' . $this->sma->formatQuantity($row->unit_quantity) . ($row->product_unit_code ? $row->product_unit_code : '') . ' x ' . ($row->item_discount != 0 ? '(' . $this->sma->formatMoney($row->unit_price + $row->item_discount) . ' - ' . $this->sma->formatMoney($row->item_discount) . ')' : $this->sma->formatMoney($row->unit_price)) . ($row->item_tax != 0 ? ' [' . lang('tax') . ' <small>(' . ($Settings->indian_gst ? $row->tax : $row->tax_code) . ')</small> ' . $this->sma->formatMoney($row->item_tax) . ($row->hsn_code ? ' (' . lang($row->product_type == 'service' ? 'sac_code' : 'hsn_code') . ': ' . $row->hsn_code . ')' : '') . ']' : '') . '</td><td class="no-border border-bottom text-right">' . $this->sma->formatMoney($row->subtotal) . '</td></tr>';
            ...
            <div style="clear:both;"></div>
            <p>Sistema de Facturacion <strong> ProNicSyst </strong> <br> Website: www.pronicsyst.com</p>
        </div>
