...
class Reports extends MY_Controller
{
    ...
    public function getPaymentsReport($pdf = null, $xls = null)
    {
        ...
        if ($pdf || $xls) {
            $this->db
                ->select('' . $this->db->dbprefix('payments') . '.date, ' . $this->db->dbprefix('payments') . '.reference_no as payment_ref, ' . $this->db->dbprefix('users') . '.username as sale_ref, ' . $this->db->dbprefix('sales') . '.customer as purchase_ref, paid_by, amount, type')
                ->from('payments')
                ->join('sales', 'payments.sale_id=sales.id', 'left')
                ->join('users', 'payments.created_by=users.id', 'left')
                ->join('purchases', 'payments.purchase_id=purchases.id', 'left')
                ->where('amount >', 0) // Solo incluye registros con amount > 0
            ...
            $this->load->library('datatables');
            $this->datatables
                ->select("DATE_FORMAT({$this->db->dbprefix('payments')}.date, '%Y-%m-%d %T') as date, " . $this->db->dbprefix('payments') . '.reference_no as payment_ref, ' . $this->db->dbprefix('users') . '.username as sale_ref, ' . $this->db->dbprefix('sales') . ".customer as purchase_ref, paid_by, amount, type, {$this->db->dbprefix('payments')}.id as id")
                ->from('payments')
                ->join('sales', 'payments.sale_id=sales.id', 'left')
                ->join('users', 'payments.created_by=users.id', 'left')
                ->join('purchases', 'payments.purchase_id=purchases.id', 'left')
    ...
    public function getPurchasesReport($pdf = null, $xls = null)
    {
        ...
        if ($pdf || $xls) {
            $this->db->select(
                ...
                $this->db->dbprefix('purchases') . '.id as reference_no, ' . 
                $this->db->dbprefix('warehouses') . '.name as wname, ' . 
                $this->db->dbprefix('purchases') . '.supplier, ' . 
                $this->db->dbprefix('brands') . '.name as brand, ' .  // Selección del nombre de la marca
                $this->db->dbprefix('users') . '.username as created_by, ' . // Selección del nombre del usuario creador
                $this->db->dbprefix('purchase_items') . '.product_name as iname, ' .
                'IF(' . $this->db->dbprefix('purchases') . '.payment_term > 0, "Credito", "Contado") as payment_type, ' . // Condicional para el tipo de pago 
                $this->db->dbprefix('purchase_items') . '.product_unit_code as unidad, ' . 
                $this->db->dbprefix('purchase_items') . '.quantity, ' . 
                $this->db->dbprefix('purchase_items') . '.net_unit_cost as unit_cost, ' . 
                '(' . $this->db->dbprefix('purchase_items') . '.quantity * ' . $this->db->dbprefix('purchase_items') . '.net_unit_cost) as grand_total, ' . // Cálculo del total
                // Asegúrate de que este campo refleje lo que realmente es el 'paid' (pagado)
                $this->db->dbprefix('purchase_items') . '.item_discount as paid, ' . // Cambiar este campo si es necesario
                $this->db->dbprefix('purchases') . '.status', 
                false // Indica que los nombres de las tablas no deben ser escapados automáticamente
            )
            ->from('purchases')
            ->join('purchase_items', 'purchase_items.purchase_id = purchases.id', 'left')
            ->join('warehouses', 'warehouses.id = purchases.warehouse_id', 'left')
            ->join('products', 'products.id = purchase_items.product_id', 'left') // Unión con la tabla de productos
            ->join('brands', 'brands.id = products.brand', 'left') // Unión con la tabla de marcas para obtener el nombre de la marca
            ->join('users', 'users.id = purchases.created_by', 'left') // Unión con la tabla de usuarios para obtener el nombre del creador
            ->order_by('purchases.date desc');
            ...
            if (!empty($data)) {
                ...
                $this->excel->getActiveSheet()->SetCellValue('A1', lang('date'));
                $this->excel->getActiveSheet()->SetCellValue('B1', lang('Factura'));
                $this->excel->getActiveSheet()->SetCellValue('C1', lang('Bodega'));
                $this->excel->getActiveSheet()->SetCellValue('D1', lang('Creado por'));
                $this->excel->getActiveSheet()->SetCellValue('E1', lang('supplier'));
                $this->excel->getActiveSheet()->SetCellValue('F1', lang('Marca'));
                $this->excel->getActiveSheet()->SetCellValue('G1', lang('product_qty'));
                $this->excel->getActiveSheet()->SetCellValue('H1', lang('Condiciones de pago'));
                $this->excel->getActiveSheet()->SetCellValue('I1', lang('Unidad de medida'));
                $this->excel->getActiveSheet()->SetCellValue('J1', lang('quantity'));
                $this->excel->getActiveSheet()->SetCellValue('K1', lang('Costo de compra'));
                $this->excel->getActiveSheet()->SetCellValue('L1', lang('subtotal'));
                ...
                foreach ($data as $data_row) {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('D' . $row, $data_row->created_by); // Nueva columna para 'Creado por'
                    $this->excel->getActiveSheet()->SetCellValue('E' . $row, $data_row->supplier);
                    $this->excel->getActiveSheet()->SetCellValue('F' . $row, $data_row->brand); // Marca
                    $this->excel->getActiveSheet()->SetCellValue('G' . $row, $data_row->iname);
                    $this->excel->getActiveSheet()->SetCellValue('H' . $row, $data_row->payment_type); // Condiciones de pago
                    $this->excel->getActiveSheet()->SetCellValue('I' . $row, $data_row->unidad); // Unidad de medida
                    $this->excel->getActiveSheet()->SetCellValue('J' . $row, $data_row->quantity); // Unidad de medida
                    $this->excel->getActiveSheet()->SetCellValue('K' . $row, $data_row->unit_cost); // Costo Unitario
                    $this->excel->getActiveSheet()->SetCellValue('L' . $row, $data_row->grand_total); // Total
                    $this->excel->getActiveSheet()->SetCellValue('M' . $row, $data_row->paid);
                    $total   += $data_row->grand_total;
                    $paid    += $data_row->paid;
                    $balance += ($data_row->grand_total - $data_row->paid);
                    $costo_unitario += $data_row->unit_cost;
                    $cantidad += $data_row->quantity;
                    $row++;
                }
                $this->excel->getActiveSheet()->getStyle('J' . $row . ':M' . $row)->getBorders()
                    ->getTop()->setBorderStyle('medium');
                $this->excel->getActiveSheet()->SetCellValue('J' . $row, $cantidad);
                $this->excel->getActiveSheet()->SetCellValue('K' . $row, $costo_unitario);
    ...
    public function getSalesReport($pdf = null, $xls = null)
    {
        ...
        if ($due_date) {
            $due_date = $this->sma->fld($due_date);
        }
        if (!$this->Owner && !$this->Admin && !$this->session->userdata('view_right')) {
            $user = $this->session->userdata('user_id');
        }
        ...
        if ($pdf || $xls) {
            $this->db
                ->select('date, sales.id as reference_no, biller, customer, GROUP_CONCAT(CONCAT(' . $this->db->dbprefix('sale_items') . ".product_name, ' (', " . $this->db->dbprefix('sale_items') . ".quantity, ')') SEPARATOR '\n') as iname, grand_total, paid, payment_status, due_date", false)
                ->from('sales')
            ...
            if ($due_date) {
                $this->db->where($this->db->dbprefix('sales') . '.due_date = "' . $due_date . '"');
            }
            ...
            if (!empty($data)) {
                ...
                $this->excel->getActiveSheet()->SetCellValue('J1', lang('due_date'));
                ...
                foreach ($data as $data_row) {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('J' . $row, $this->sma->hrld($data_row->due_date));
                    $total   += $data_row->grand_total;
                    $paid    += $data_row->paid;
                    $balance += ($data_row->grand_total - $data_row->paid);
                    $row++;
            ...
            $this->load->library('datatables');
            $this->datatables
                ->select("DATE_FORMAT(date, '%Y-%m-%d %T') as date, sales.id as reference_no, biller, customer, FSI.item_nane as iname, grand_total, paid, (grand_total-paid) as balance, payment_status, DATE_FORMAT(due_date, '%Y-%m-%d %T') as due_date, {$this->db->dbprefix('sales')}.id as id", false)
                ->from('sales')
            ...
            if ($due_date) {
                $this->datatables->where($this->db->dbprefix('sales') . '.due_date = "' . $due_date . '"');
            }
    ...
    public function products_x_usuario()
    {
        $this->sma->checkPermissions();
        $this->data['error']      = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
        $this->data['categories'] = $this->site->getAllCategories();
        ...
        $bc   = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('reports'), 'page' => lang('reports')], ['link' => '#', 'page' => lang('products_report')]];
        $meta = ['page_title' => lang('products_report'), 'bc' => $bc];
        $this->page_construct('reports/products_x_usuario', $meta, $this->data);
    }

    public function getProducts_x_usuario_Report($pdf = null, $xls = null)
    {
        $this->sma->checkPermissions('products', true);
        ...
        $pp = "( SELECT product_id, p.date as date, p.created_by as created_by, 
        ...
        on p.id = pi.purchase_id WHERE pi.status = 'received' ";
        // WHERE p.status != 'pending' AND p.status != 'ordered'
        $sp = '( SELECT u.username as username, si.product_id, s.date as date, s.created_by as created_by, 
        SUM( si.quantity ) soldQty, SUM( si.quantity * si.sale_unit_price ) totalSale 
        from ' . $this->db->dbprefix('costing') . ' si 
        JOIN ' . $this->db->dbprefix('sales') . ' s
        on s.id = si.sale_id
        JOIN ' . $this->db->dbprefix('users') . ' u
        ...
        if ($pdf || $xls) {
            $this->db
                ->select($this->db->dbprefix('products') . '.code, ' . $this->db->dbprefix('products') . '.name,
                COALESCE( PSales.soldQty, 0 ) as SoldQty,
                COALESCE( cost, 0 ) as PurchasedQty,
                COALESCE( price, 0 ) as BalacneQty,
                COALESCE( PSales.soldQty * cost, 0 ) as TotalPurchase,
                COALESCE( PSales.totalSale, 0  ) as TotalBalance,
                (COALESCE( PSales.totalSale, 0 ) - COALESCE( PSales.soldQty * cost, 0 )) as TotalSales,
                
                
                COALESCE( PCosts.balacneQty, 0 ) as Profit, PSales.username,', false)
                ->from('products')
                ->join($sp, 'products.id = PSales.product_id', 'left')
                ->join($pp, 'products.id = PCosts.product_id', 'left')
                ->where('products.type !=', 'combo')
            ...
            if (!empty($data)) {
                ...
                $this->excel->getActiveSheet()->SetCellValue('F1', lang('Total Costo'));
                $this->excel->getActiveSheet()->SetCellValue('G1', lang('Total Vendido'));
                $this->excel->getActiveSheet()->SetCellValue('H1', lang('Utilidad'));
                $this->excel->getActiveSheet()->SetCellValue('I1', lang('stock_in_hand'));
                ...
                foreach ($data as $data_row) {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('E' . $row, $data_row->BalacneQty);
                    $this->excel->getActiveSheet()->SetCellValue('F' . $row, $data_row->TotalPurchase);
                    $this->excel->getActiveSheet()->SetCellValue('G' . $row, $data_row->TotalBalance);
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('J' . $row, $data_row->username);
                ...
                $this->excel->getActiveSheet()->SetCellValue('E' . $row, $bQty);
                $this->excel->getActiveSheet()->SetCellValue('F' . $row, $pAmt);
                $this->excel->getActiveSheet()->SetCellValue('G' . $row, $bAmt);
                ...
                $filename = 'products_x_usuario_report';
                $this->load->helper('excel');
                create_excel($this->excel, $filename);
            }
            $this->session->set_flashdata('error', lang('nothing_found'));
            ...
            $this->datatables
                ->select($this->db->dbprefix('products') . '.code, ' . $this->db->dbprefix('products') . ".name,
                COALESCE( PSales.soldQty, 0 ) as cantidad,
                cost, 
                price,
                CONCAT(COALESCE( PSales.soldQty, 0 ), '__', COALESCE( PSales.soldQty * cost, 0 )) as purchased,
                CONCAT(COALESCE( PSales.soldQty, 0 ), '__', COALESCE( PSales.totalSale, 0 )) as sold,
                (COALESCE( PSales.totalSale, 0 ) - COALESCE( PSales.soldQty * cost, 0 )) as Profit,
                COALESCE( PCosts.balacneQty, 0 ) as balance,
                PSales.username,
    ...
    public function sales1()
    {
        ...
        $bc                       = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('reports'), 'page' => lang('reports')], ['link' => '#', 'page' => lang('sales_report')]];
        $meta                     = ['page_title' => lang('sales_report'), 'bc' => $bc];
        $this->page_construct('reports/sales1', $meta, $this->data);
    }
    public function getSalesReport1($pdf = null, $xls = null)
    {
        $this->sma->checkPermissions('sales', true);
        $product      = $this->input->get('product') ? $this->input->get('product') : null;
        $user         = $this->input->get('user') ? $this->input->get('user') : null;
        ...
        if ($pdf || $xls) {
            ...
            sales.id, 
            warehouses.name as bodega, 
            username, 
            brands.name as marca, 
            products.name as iname, 
            companies.name as customer, 
            sales.staff_note as city, 
            sales.payment_status as payment_status,
            products.cost as costo, 
            sale_items.product_unit_code as unit_code, 
            sale_items.quantity as quantity, 
            sale_items.unit_price as price, 
            sale_items.quantity*price as sub_total, 
            sale_items.discount as descuento,
            ...
            if ($start_date) {
                ...
                $fecha_inicio = $fecha_actual->modify('-30 days')->format('Y-m-d') . ' 00:00:00'; // 30 días atrás, a las 00:00
                $fecha_fin = (new DateTime())->format('Y-m-d') . ' 23:59:59'; // Hoy, a las 23:59

                // Aplicamos la condición
                $this->db->where($this->db->dbprefix('sales') . '.date BETWEEN "' . $fecha_inicio . '" AND "' . $fecha_fin . '"');
            }
            ...
            if (!empty($data)) {
                ...
                // Definimos el orden de las columnas según la consulta
                $this->excel->getActiveSheet()->SetCellValue('A1', lang('date'));
                $this->excel->getActiveSheet()->SetCellValue('B1', lang('NoFact'));
                $this->excel->getActiveSheet()->SetCellValue('C1', lang('bodega'));
                $this->excel->getActiveSheet()->SetCellValue('D1', lang('username'));
                $this->excel->getActiveSheet()->SetCellValue('E1', lang('marca'));
                $this->excel->getActiveSheet()->SetCellValue('F1', lang('products'));
                $this->excel->getActiveSheet()->SetCellValue('G1', lang('customer'));
                $this->excel->getActiveSheet()->SetCellValue('H1', lang('city'));
                $this->excel->getActiveSheet()->SetCellValue('I1', lang('payment_status'));
                $this->excel->getActiveSheet()->SetCellValue('J1', lang('costo'));
                $this->excel->getActiveSheet()->SetCellValue('K1', lang('unit'));
                $this->excel->getActiveSheet()->SetCellValue('L1', lang('quantity'));
                $this->excel->getActiveSheet()->SetCellValue('M1', lang('price'));
                $this->excel->getActiveSheet()->SetCellValue('N1', lang('sub_total'));
                $this->excel->getActiveSheet()->SetCellValue('O1', lang('descuento'));
                ...
                foreach ($data as $data_row) {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('C' . $row, $data_row->bodega);
                    $this->excel->getActiveSheet()->SetCellValue('D' . $row, $data_row->username);
                    $this->excel->getActiveSheet()->SetCellValue('E' . $row, $data_row->marca);
                    $this->excel->getActiveSheet()->SetCellValue('F' . $row, $data_row->iname);
                    $this->excel->getActiveSheet()->SetCellValue('G' . $row, $data_row->customer);
                    $this->excel->getActiveSheet()->SetCellValue('H' . $row, $data_row->city);
                    $this->excel->getActiveSheet()->SetCellValue('I' . $row, lang($data_row->payment_status));
                    $this->excel->getActiveSheet()->SetCellValue('J' . $row, $data_row->costo);
                    $this->excel->getActiveSheet()->SetCellValue('K' . $row, $data_row->unit_code);
                    $this->excel->getActiveSheet()->SetCellValue('L' . $row, $data_row->quantity);
                    $this->excel->getActiveSheet()->SetCellValue('M' . $row, $data_row->price);
                    $this->excel->getActiveSheet()->SetCellValue('N' . $row, $data_row->sub_total);
                    $this->excel->getActiveSheet()->SetCellValue('O' . $row, $data_row->descuento);
                    
                    $paid += $data_row->quantity;
                    $balance += $data_row->sub_total;
                    $row++;
                }
            
                // Estilo y ajustes de columnas
                ...
                $this->excel->getDefaultStyle()->getAlignment()->setVertical('center');
                $this->excel->getActiveSheet()->getStyle('H2:H' . $row)->getAlignment()->setWrapText(true);
            
                $filename = 'Reporte_de_ventas_por_item';
                $this->load->helper('excel');
                create_excel($this->excel, $filename);
            }            
            $this->session->set_flashdata('error', lang('nothing_found'));
            ...
            if ($start_date) {
                ...
                // Aplicamos la condición
                $this->datatables->where($this->db->dbprefix('sales') . '.date BETWEEN "' . $fecha_inicio . '" AND "' . $fecha_fin . '"');
            }
    ...
    public function getSalesCartera($pdf = null, $xls = null)
    {
        $this->sma->checkPermissions('payments', true);
        ...
        if ($pdf || $xls) {
            $this->db->select("
                ...
                DATE_FORMAT({$this->db->dbprefix('sales')}.date, '%Y-%m-%d %T') as date_sale,
                {$this->db->dbprefix('payments')}.id as payment_ref, 
                {$this->db->dbprefix('sales')}.id as sale_ref, 
                {$this->db->dbprefix('companies')}.name as customer, 
                {$this->db->dbprefix('users')}.username as created_by,
                CASE 
                    WHEN {$this->db->dbprefix('sales')}.payment_term > 0 THEN 'Crédito' 
                    ELSE 'Contado' 
                END AS payment_type, 
                sma_sales.payment_term as dias_credito,
                sma_sales.total as total_factura,
                amount as abono, 
                (sma_sales.total - sma_sales.paid) as saldo, 
                DATE_FORMAT(CURDATE(), '%Y-%m-%d %T') as dia_hoy,
                CASE 
                    WHEN sma_sales.due_date IS NOT NULL THEN DATEDIFF(CURDATE(), sma_sales.due_date) 
                    ELSE 0 
                END AS atraso
            ")
            ->from('payments')
            ...
            if ($start_date) {
                ...
                // Calculamos las fechas
                $fecha_actual = new DateTime(); // Fecha actual
                $fecha_inicio = $fecha_actual->modify('-7 days')->format('Y-m-d') . ' 00:00:00'; // 30 días atrás, a las 00:00
                $fecha_fin = (new DateTime())->format('Y-m-d') . ' 23:59:59'; // Hoy, a las 23:59
            ...
            if (!empty($data)) {
                ...
                $this->excel->setActiveSheetIndex(0);
                $this->excel->getActiveSheet()->setTitle(lang('payments_report'));
                $this->excel->getActiveSheet()->SetCellValue('A1', lang('Fecha Pago'));
                $this->excel->getActiveSheet()->SetCellValue('B1', lang('Fecha Factura'));
                $this->excel->getActiveSheet()->SetCellValue('C1', lang('No Pago'));
                $this->excel->getActiveSheet()->SetCellValue('D1', lang('Factura'));
                $this->excel->getActiveSheet()->SetCellValue('E1', lang('Cliente'));
                $this->excel->getActiveSheet()->SetCellValue('F1', lang('Creado por'));
                $this->excel->getActiveSheet()->SetCellValue('G1', lang('Condiciones Pago'));
                $this->excel->getActiveSheet()->SetCellValue('H1', lang('Días Crédito'));
                $this->excel->getActiveSheet()->SetCellValue('I1', lang('Monto Factura'));
                $this->excel->getActiveSheet()->SetCellValue('J1', lang('Abonos'));
                $this->excel->getActiveSheet()->SetCellValue('K1', lang('Saldo'));
                $this->excel->getActiveSheet()->SetCellValue('L1', lang('Hoy'));
                $this->excel->getActiveSheet()->SetCellValue('M1', lang('Atraso'));
            
                $row = 2;
                foreach ($data as $data_row) {
                    $this->excel->getActiveSheet()->SetCellValue('A' . $row, $data_row->date);
                    $this->excel->getActiveSheet()->SetCellValue('B' . $row, $data_row->date_sale);
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('G' . $row, $data_row->payment_type);
                    $this->excel->getActiveSheet()->SetCellValue('H' . $row, $data_row->dias_credito);
                    $this->excel->getActiveSheet()->SetCellValue('I' . $row, $data_row->total_factura);
                    $this->excel->getActiveSheet()->SetCellValue('J' . $row, $data_row->abono);
                    $this->excel->getActiveSheet()->SetCellValue('K' . $row, $data_row->saldo);
                    $this->excel->getActiveSheet()->SetCellValue('L' . $row, $data_row->dia_hoy);
                    $this->excel->getActiveSheet()->SetCellValue('M' . $row, $data_row->atraso);
                    $row++;
                }
    ...
    public function sales_cartera()
    {
        ...
        $this->data['pos_settings'] = POS ? $this->reports_model->getPOSSetting('biller') : false;
        $bc                         = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('reports'), 'page' => lang('reports')], ['link' => '#', 'page' => lang('payments_report')]];
        $meta                       = ['page_title' => lang('Reporte Cartera'), 'bc' => $bc];
        $this->page_construct('reports/sales_cartera', $meta, $this->data);
    }
