...
class Transfers extends MY_Controller
{
        ...
        if ($this->form_validation->run() == true) {
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('H1', lang('unit_code'));
                    ...
                    foreach ($_POST['val'] as $id) {
                        $transfers = $this->transfers_model->getTransferByID_1($id);
                    
                        if (!$transfers) {
                            log_message('error', "No data found for transfer ID: $id");
                            continue;
                        }
                
                        if ($transfers) {
                            foreach ($transfers as $transfer) { // Acceso a datos como propiedades del objeto
                                $this->excel->getActiveSheet()->SetCellValue('A' . $row, $this->sma->hrld($transfer->date));
                                $this->excel->getActiveSheet()->SetCellValue('B' . $row, $transfer->transfer_no);
                                $this->excel->getActiveSheet()->SetCellValue('C' . $row, $transfer->fname);
                                $this->excel->getActiveSheet()->SetCellValue('D' . $row, $transfer->tname);
                                $this->excel->getActiveSheet()->SetCellValue('E' . $row, $transfer->username);
                                $this->excel->getActiveSheet()->SetCellValue('F' . $row, $transfer->brand);
                                $this->excel->getActiveSheet()->SetCellValue('G' . $row, $transfer->product);
                                $this->excel->getActiveSheet()->SetCellValue('H' . $row, $transfer->product_unit_code);
                                $this->excel->getActiveSheet()->SetCellValue('I' . $row, $transfer->cantidad);
                                $this->excel->getActiveSheet()->SetCellValue('J' . $row, $transfer->costo);
                                $this->excel->getActiveSheet()->SetCellValue('K' . $row, $transfer->cantidad * $transfer->costo);
                                $this->excel->getActiveSheet()->SetCellValue('L' . $row, $transfer->status);
                                $row++;
                            }
                        }
                    }
    ...
    public function index_1()
    {
        ...
        $bc   = [['link' => base_url(), 'page' => lang('home')], ['link' => '#', 'page' => lang('Productos_transferidos')]];
        $meta = ['page_title' => lang('Productos_trasnferidos'), 'bc' => $bc];
        $this->page_construct('transfers/index_1', $meta, $this->data);
    }
    public function getTransfers_1()
    {
        $this->sma->checkPermissions('index');
        ...
        $this->datatables
            ...
            subtotal, transfers.status, transfers.attachment')
            ->from('purchase_items')
